...
export function CreatorCard({ c }: { c: MockCreator }) {
  ...
  return (
    <Link
      to={`/c/${c.handle}`}
      className="group flex flex-col gap-4 rounded-xl border border-white/[0.06] bg-surface p-5 transition-all hover:border-brand-purple/40 hover:shadow-glow"
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-display text-base font-semibold leading-tight">${c.tokenSymbol}</div>
      ...
      <div className="mt-auto flex items-end justify-between gap-3 pt-2">
        <div>
          <div className="label-eyebrow text-tertiary-fg">Price</div>
          <div className="font-mono-num text-base">{formatSOL(c.priceSOL, 4)} SOL</div>
        </div>
        <span className={`rounded-md px-2 py-1 text-xs font-semibold ${up ? "pill-up" : "pill-down"}`}>
          {up ? "+" : ""}
